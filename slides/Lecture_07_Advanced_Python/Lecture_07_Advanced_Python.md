---
title: Lecture_07_Advanced_Python
separator: <!--s-->
verticalSeparator: <!--v-->
theme: simple
...
makeTitle:
  ...
  title: Python 进阶
  detail: SI100+ 2026 Staff | 2026-08-13
makeThanks: True
revealOptions:
  transition: "slide"
...
Notebook 适合探索，但到实际的项目中，我们就需要去规划代码结构了：
...
- 相关模块可以放进同一个**包 (package)**
- `pyproject.toml` 描述项目及其依赖，uv 根据它创建可复现的环境
...
- `import module`：导入模块，通过 `module.name` 访问成员
- `from module import name`：直接导入需要的名称
- 避免 `from module import *`，否则名称来源会变得模糊
- 导入模块时，模块顶层代码会执行一次
...
from weather import convert       # 项目内代码
```

- **标准库**随 Python 提供，不需要单独安装
- **第三方包**来自 PyPI，应作为项目依赖管理
- **项目内模块**由我们自己维护
...
不要在已经由 uv 管理的项目中直接使用裸 `pip install`。

<!--v-->

## 用 `pathlib` 处理文件路径

旧代码常用 `os.path` 拼接字符串；而现在更主流的是使用 `pathlib.Path`：
...
report_path.write_text("SI100+\n", encoding="utf-8")
print(report_path.read_text(encoding="utf-8"))
```

- `/` 负责组合路径，不需要手写 Windows 或 macOS/Linux 分隔符
- 相对路径以程序的**当前工作目录**为基准
- `Path.cwd()` 可以查看当前工作目录
...
- `zip` 将多个可迭代对象中对应位置的元素配对
- Python 3.10+ 可用 `strict=True` 检查长度是否一致
- 默认情况下，`zip` 会在最短的输入结束时停止
...
**演示**：Notebook 示例 7.4.1, 7.4.2, 7.4.3
...
- **名称改写 (name mangling)** ：`self.__baz` (双下划线开头)
  - Python 会将它改写为类似 `_ClassName__baz` 的名称，主要用于避免子类意外覆盖；它不是不可访问的严格私有机制。

<br/>

**演示**: Notebook 示例 7.5.1
...
- 项目环境：标准库直接使用，第三方包交给 uv 管理
- `pathlib`、`enumerate`、`zip` 和推导式：让常见操作更直接
